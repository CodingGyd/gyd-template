...
import com.gydblog.toolkit.config.GenConfig;
import com.gydblog.common.utils.StrUtil;
import com.gydblog.toolkit.mapper.TableMapper;
import com.gydblog.toolkit.util.BuilderUtil;
import com.gydblog.toolkit.util.GenUtil;
import com.gydblog.toolkit.util.VelocityInitializer;
import com.gydblog.toolkit.util.VelocityUtil;
import com.gydblog.common.constant.CommonConstants;
import com.gydblog.common.constant.GenConstants;
import com.gydblog.common.constant.SqlConstants;
...
import static com.gydblog.toolkit.util.GenUtil.*;
    ...
    public Table getGenTable(String tableName, String tableComment) {
        ...
        if (StrUtil.isBlank(tableName)) {
            ...
            if (table != null) {
                ...
                table.setGenApi("0");
                table.setGenWay("1");
                table.setCreateUser(SecurityUtils.getUserId());
    ...
    public void validateEdit(Table table) {
        if (GenConstants.TPL_TREE.equals(table.getTplCategory())) {
            ...
            if (StrUtil.isEmpty(paramsObj.getStr(GenConstants.TREE_ID))) {
                throw new RuntimeException("树编号字段不能为空");
            } else if (StrUtil.isEmpty(paramsObj.getStr(GenConstants.TREE_PARENT_ID))) {
                throw new RuntimeException("树父编号字段不能为空");
        ...
        if (StrUtil.isBlankIfStr(table.getId())) {
            ...
            if (row > 0) {
                ...
                if (oldTable != null) {
                    ...
                    //方案二：备份原表重建
                    newTableName = oldTable.getTableName() + DateUtil.format(new Date(), DatePattern.PURE_TIME_PATTERN);
                ...
                try {
                    ...
                    if (StrUtil.isNotEmpty(newTableName)) {
                        //删除备份表
                        jdbcTemplate.execute("drop table "+ newTableName + ";");
                    }
                }catch (Exception e){
                    //更新失败，回滚
                    if (StrUtil.isNotEmpty(newTableName)) {
                        ...
                        jdbcTemplate.execute("rename  table " + newTableName + " to " + oldTable.getTableName() + ";");
                    }
                    throw e;
                }
        ...
        if (table != null) {
            ...
            for (String template : templates) {
                ...
                if (StrUtil.containsAny(template, "vue", "js")) {
                    localFile = table.getGenPath().replace("\\", "/") + "/gyd-ui/" + VelocityUtil.getFileName(template, table).replace("vue/", "src/");
                } else if (StrUtil.contains(template, "sql")) {
                    localFile = table.getGenPath().replace("\\", "/") + "/gyd-biz/gyd-" + table.getModuleName() + "/sql/" + VelocityUtil.getFileName(template, table);
                    sqlPath = localFile;
                } else {
                    localFile = table.getGenPath().replace("\\", "/") + "/gyd-biz/gyd-" + table.getModuleName() + "/src/" + VelocityUtil.getFileName(template, table);
                }
                IoUtil.write(FileUtil.getOutputStream(localFile), CommonConstants.UTF8, false, sw.toString());
                IoUtil.close(sw);
                //查询菜单是否创建
                List<Map<String, Object>> maps = jdbcTemplate.queryForList("select * from sys_menu where menu_name = ? and del_flag='0'", table.getFunctionName());
                if (maps.size() == 0) {
                    //执行生成菜单脚本
