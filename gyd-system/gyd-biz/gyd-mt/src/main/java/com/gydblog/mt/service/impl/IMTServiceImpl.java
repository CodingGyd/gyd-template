...
import com.gydblog.mt.mapper.IAppointRecordMapper;
import com.gydblog.mt.mapper.IUserMapper;
import com.gydblog.mt.service.IMTLogFactory;
import com.gydblog.mt.service.IMTService;
import com.gydblog.mt.service.IShopService;
import com.gydblog.mt.service.IUserService;
import com.gydblog.common.core.RedisCache;
import com.gydblog.common.enums.AppointRecordStatusEnum;
import com.gydblog.common.enums.AppointRecordTypeEnum;
...
public class IMTServiceImpl implements IMTService {
    ...
    @Autowired
    private IAppointRecordMapper iAppointRecordMapper;
...
//        List<IUser> iUsers = iUserService.selectReservationUserByMinute(minute);
        ...
        for (IUser iUser : iUsers) {
            record = new IAppointRecord();
            record.setRecordDate(curDate);
            record.setRecordType(AppointRecordTypeEnum.MAO_TAI.getCode());
            record.setUserId(iUser.getUserId());
            record.setMobile(iUser.getMobile());
            record.setRecordTime(new Date());
            if (iAppointRecordMapper.selectOne(record) != null) {
                logger.info("「该用户今日已预约，跳过」" + iUser.getUserId());
                continue;
            }
            //判断当日是否已预约过
            ...
            //预约日志记录
            record.setStatus(AppointRecordStatusEnum.NORMAL.getCode());
            record.setContent("预约操作完成");
            iAppointRecordMapper.insert(record);
            //延时3秒
            try {
